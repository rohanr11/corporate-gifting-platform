      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-2">
          <Link to={`/products/${product.slug}`}>
            <h3 className="font-display text-base font-semibold leading-snug hover:text-brass transition-colors">
              {product.name}
            </h3>
          </Link>
          <button
            ...
            className="shrink-0 text-porcelain/40 hover:text-burgundy transition-colors"
          >
        ...
        {reason && (
          <p className="text-xs text-porcelain/80 mt-2 italic border-l-2 border-brass pl-2">
            {reason}
          </p>
        )}

        <div className="mt-auto pt-3 flex items-center justify-between">
          <span className="font-mono font-semibold text-porcelain">₹{product.price.toLocaleString("en-IN")}</span>
